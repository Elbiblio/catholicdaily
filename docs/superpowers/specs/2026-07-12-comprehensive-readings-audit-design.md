...
Universalis is not a public multi-year web archive. The public web site documents that it shows yesterday, today, and the week ahead; outside that window it serves an "Other dates" page directing users to its paid apps/programs for unlimited dates. Therefore the Universalis fetch adapter must fail loudly when a fetched page contains no Mass reading sections. For future dates beyond the public window, use local extract fixtures, cached/user-provided pages, or another source adapter rather than silently accepting a zero-entry catalog.
